...
function saveSettings() {
  localStorage.setItem("affectle-nightmode", document.querySelector("body").classList.contains("nightmode") ? '1' : '0')
  localStorage.setItem("affectle-colorblind", document.querySelector("body").classList.contains("colorblind") ? '1' : '0')
}

function restoreSettings() {
  const nightmode = parseInt(localStorage.getItem("affectle-nightmode") || '0')
  const colorblind = parseInt(localStorage.getItem("affectle-colorblind") || '0')
...
function createUnicodeGameTranscript() {
  ...
  if (transcript.endsWith(greenSquare+greenSquare+greenSquare+greenSquare+greenSquare)) {
    return "affectle #" + (targetWords.indexOf(targetWord) + 1) + " " + usedRows + "/6\n" + transcript
  } else {
    return "affectle #" + (targetWords.indexOf(targetWord) + 1) + " X/6\n" + transcript
  }
}
